...
import { FileName } from './docs/type'

const getModuleName = (root: string) => (path: string) => {
  const paths = _path.relative(root, path);
  return _path.parse(paths).dir.split(_path.sep).join('.') + '.' + _path.parse(paths).name
};
export const readFiles = (mark:string)=>(root: string, path: string, outpath: string): IO<FileName[]> =>
  filesSync(_path.resolve(root, path))(mark).map(paths =>
    paths.map(p => ({
...
export const writeModule = (path: string,name: string) => ( content: string): IO<void> =>
  writeFileWithStringSync(path,name)(content);
export const writeFile = (path: string,name: string) => ( content: string): IO<void> =>
  writeFileWithStringSync(path,name)(content);
export const removeExtendName = (name: String, extendName: string) => name.substring(0, name.lastIndexOf('.' + extendName))

export const readApp = (root: string, path: string): IO<string> =>{
 return  readFileAsPlainStringSync(_path.resolve(root, path, 'info.json'));
}
