...
const response =
    (name: t.GlobalPropFunction['media']['name'], level: t.GlobalPropFunction['media']['level']) =>
        (value: number) => {
            return ({
                media: ({
                    level,
...
const responseWidth = (value: number) => ({ width: `${(value / 12) * 100}%` });
const defaultTransitionSetting: t.DefaultTransitionSetting = {
    duration: 'shorter',
    ease: 'easeInOut',
    delay: 'none',
};
const inherit = (name: string) => ({ mkstyle: { [name]: 'inherit' } });
const media = (a: t.GlobalPropFunction['media']) => (v: variable.GlobalType.GlobalVariable) => {
    const value = variable.variable.getVariableValue({
        kind: 'breakpoints',
        level: a.name,
    })(v as any) as number;
    ...
    return ({
        selector: [{
            name: `@media (${a.level === 'max' ? 'max' : 'min'}-width: ${vValue}px)`,
            value: a.value,
        }],
    });

};
const M = M1.addProps<t.GlobalProp, t.GlobalPropFunction>({
    color: {
    ...
    bgColor: {
        transparent: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
        ...
        paper: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
                backgroundColor: { name: 'light', level: 'full', kind: 'ColorCommon' },
            },
        },
        content: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
                backgroundColor: { name: 'grey', level: 50, kind: 'PaletteMonoColor' },
            },
        },
        divider: {
        ...
        main: {
            mkstyle: {
                color: { name: 'light', level: 'full', kind: 'ColorCommon' },
                backgroundColor: { name: 'first', level: 'normal', kind: 'Color' },
            },
        },
        success: {
            mkstyle: {
                color: { name: 'light', level: 'full', kind: 'ColorCommon' },
                backgroundColor: { name: 'success', level: 'normal', kind: 'Color' },
            },
        },
        warning: {
            mkstyle: {
                color: { name: 'light', level: 'full', kind: 'ColorCommon' },
                backgroundColor: { name: 'warning', level: 'normal', kind: 'Color' },
            },
        },
        alert: {
            mkstyle: {
                color: { name: 'light', level: 'full', kind: 'ColorCommon' },
                backgroundColor: { name: 'alert', level: 'normal', kind: 'Color' },
            },
        },
        accent: {
            mkstyle: {
                color: { name: 'light', level: 'full', kind: 'ColorCommon' },
                backgroundColor: { level: 'normal', kind: 'ColorAccent' },
            },
        },
        alt: {
            mkstyle: {
        ...
        disabled: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
                backgroundColor: { name: 'dark', level: 'large', kind: 'ColorCommon' },
            },
        },
        selected: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
                backgroundColor: { name: 'grey', level: 200, kind: 'PaletteMonoColor' },
            },
        },
        focus: {
            mkstyle: {
                color: { name: 'dark', level: 'largeXXX', kind: 'ColorCommon' },
                backgroundColor: { name: 'grey', level: 300, kind: 'PaletteMonoColor' },
            },
        },
    ...
    width: {
        ...
        third: {
            mkstyle: { width: { kind: 'width', level: 'third' } },
        },
    ...
    columns3: {
        ...
        transition: withDefaultsF<DefaultTransitionSetting, TransitionSetting, any>
            (defaultTransitionSetting)(v => (v1: variable.GlobalType.GlobalVariable) => {
                const duration = variable.variable.getVariableValue({
                    kind: 'duration',
                    level: v.duration,
                })(v1 as any) as number;
                const ease = variable.variable.getVariableValue({
                    kind: 'ease',
                    level: v.ease,
                })(v1 as any);
                const delay = variable.variable.getVariableValue({
                    kind: 'duration',
                    level: v.delay,
                })(v1 as any) as number;
                return ({
        ...
        boxShadow: (a: t.GlobalPropFunction['boxShadow']) => (v: variable.GlobalType.GlobalVariable) => {
            const value = variable.variable.getVariableValue({
                kind: 'shadows',
                level: a,
            })(v as any) as number;
        ...
        lg: minMD,
        xl: minLG,
        smSetting: (value: t.InputProp) => media({ name: 'sm', level: 'max', value }),
        mdSetting: (value: t.InputProp) => media({ name: 'md', level: 'max', value }),
        xlSetting: (value: t.InputProp) => media({ name: 'xl', level: 'max', value }),
        lgSetting: (value: t.InputProp) => media({ name: 'lg', level: 'max', value }),
        smRSetting: (value: t.InputProp) => media({ name: 'sm', level: 'min', value }),
        mdRSetting: (value: t.InputProp) => media({ name: 'md', level: 'min', value }),
        xlRSetting: (value: t.InputProp) => media({ name: 'xl', level: 'min', value }),
        lgRSetting: (value: t.InputProp) => media({ name: 'lg', level: 'min', value }),
        textTransform: (a: t.GlobalPropFunction['textTransform']) => ({
            mkstyle: {
                textTransform: a,
            },
        }),
        textAlign: (a: t.GlobalPropFunction['textAlign']) => ({
            mkstyle: {
                textAlign: a,
            },
        }),
        direction: (a: t.GlobalPropFunction['direction']) => ({
            mkstyle: {
                direction: a,
            },
        }),
        textDecoration: (a: t.GlobalPropFunction['textDecoration']) => ({
            mkstyle: {
                textDecoration: a,
            },
        }),
        flexDirection: (a: t.GlobalPropFunction['flexDirection']) => ({
            mkstyle: {
                flexDirection: a,
            },
        }),
        flexWrap: (a: t.GlobalPropFunction['flexWrap']) => ({
            mkstyle: {
                flexWrap: a,
            },
        }),
        justifyContent: (a: t.GlobalPropFunction['justifyContent']) => ({
            mkstyle: {
                justifyContent: a,
            },
        }),
        alignItems: (a: t.GlobalPropFunction['alignItems']) => ({
            mkstyle: {
                alignItems: a,
            },
        }),
        order: (a: t.GlobalPropFunction['order']) => ({
            mkstyle: {
                order: a,
            },
        }),
