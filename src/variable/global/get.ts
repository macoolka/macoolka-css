...
export const isNodeValue = (a: any): a is t.GlobalNodeValue => !!(a && a.kind && nodeKinds.includes(a.kind));

export const getVariableValue = (node: t.GlobalNodeValue) =>
    (theme: t.GlobalVariable): string | number => {
        switch (node.kind) {
            case 'unit':
